...
for line in lines:
	blocks = line.split('-')
	sector, checksum = blocks[-1].split('[')
	checksum = checksum[:-1]  # remove trailing ']'

	rooms.append({'letters': ''.join(blocks[:-1]),
				  'sector': int(sector),
				  'checksum': checksum})
...
for room in rooms:
	...
	counts = {k: len(list(g)) for k, g in groupby(sorted(room['letters']))}
	# sort by counts first, then by letter
	sorted_counts = sorted(counts.items(), key=lambda x: (-x[1], x[0]))
	# the real checksum is the 5 most common letters concatenated
	real_checksum = ''.join([l for l, _ in sorted_counts][:5])

	if room['checksum'] == real_checksum:
		sum += room['sector']
