...
def supports_tls(input):
	def contains_abba(s):
		""" Returns True if the string contains an ABBA, False otherwise. """
		def is_abba(s):
			assert(len(s) == 4)
	...
	# TODO: changing this to a for loop should speed things up
	abba_in_hyper = any(contains_abba(hyper) for hyper in hypers)
	if abba_in_hyper:
		return False
	abba_in_ip = any(contains_abba(ip) for ip in ips)
	if abba_in_ip:
		return True
	return False
